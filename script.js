...
function setupLoginForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    .then((data) => {
      ...
      if (data.success) {
        localStorage.setItem(STORAGE_KEYS.activeUser, data.username);
        clearErrors(form);
        hydrateSharedPanels();
        setStatus(status, data.message || `Login successful. Welcome back, ${data.username}.`, true);
      } else {
        if (data.field) {
          setFieldError(form.elements[data.field], data.message);
        }
        setStatus(status, data.message || "Login failed. Check your username and password.", false);
      }
    })
    .catch(() => {
      submitButton.disabled = false;
      submitButton.textContent = "Login";
      // Fallback to localStorage if PHP backend is unavailable
      const users = readStorage(STORAGE_KEYS.users);
      ...
      clearErrors(form);
      hydrateSharedPanels();
      setStatus(status, `Login successful. Welcome back, ${matchedUser.username}. (local mode)`, true);
    });
  });
}
